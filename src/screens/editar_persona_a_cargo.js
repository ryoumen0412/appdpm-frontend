...
import {
  actualizarPersona,
  obtenerPersonaLocal,
} from "../api/personas_a_cargo";
import { puede } from "../utils/permisos";
import HeaderBar from "../components/header_bar";

export default function EditarPersonaACargo({
  usuario,
  onNavigate,
  onLogout,
      ...
      try {
        ...
        if (persona) {
          ...
          if (persona.fecha_nacimiento) {
            const m = /^([0-9]{2})\/([0-9]{2})\/([0-9]{4})/.exec(
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.denegadoContainer}>
          <Text style={styles.denegado}>
            No autorizado para editar personas.
          </Text>
        </View>
      </View>
    );
  ...
  const handleSubmit = async () => {
    ...
    setEnviando(true);
    try {
      await actualizarPersona(registroSeleccionadoRut, form);
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.container}>
        <Text style={styles.title}>
          Editar Persona ({registroSeleccionadoRut})
        </Text>
        {["nombre", "apellido", "correo_electronico", "telefono"].map(
          (campo) => (
            <TextInput
              key={campo}
              style={styles.input}

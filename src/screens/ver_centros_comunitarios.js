...
export default function Ver_centros_comunitarios({
  usuario,
  onLogout,
  onNavigate,
}) {
  ...
  const cargarCentros = useCallback(async (page = 1) => {
    try {
      setCargando(true);
      const data = await fetchCentros_comunitarios({ page });
      setDatos(data.items ?? []);
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.container}>
        ...
        {cargando && (
          <Text style={styles.loading}>Cargando centros comunitaros...</Text>
        )}
        {error && <Text style={styles.error}>{error}</Text>}
        {!cargando && !error && (
          <Tabla_centros_comunitarios
            data={datos}

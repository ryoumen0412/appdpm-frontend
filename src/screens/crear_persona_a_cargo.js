...
export default function CrearPersonaACargo({ usuario, onNavigate, onLogout }) {
  ...
  if (!puede(usuario,'crear_persona')) {
    return (
      <View style={{ flex:1 }}>
        ...
        <View style={styles.denegadoContainer}><Text style={styles.denegado}>No autorizado para crear personas.</Text></View>
      </View>
    );
  }
  ...
  const validar = () => {
    ...
    if (form.correo_electronico && !/.+@.+\..+/.test(form.correo_electronico)) return 'Correo inválido';
    if (form.fecha_nacimiento && !/^\d{4}-\d{2}-\d{2}$/.test(form.fecha_nacimiento)) return 'Fecha debe ser YYYY-MM-DD';
    return null;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await crearPersona(form, token);
      Alert.alert('Éxito','Persona creada', [ { text: 'OK', onPress: () => onNavigate('ver_personas_a_cargo') } ]);
    } catch (e) {
      setError(e.message);
    } finally {
      setEnviando(false);
  ...
  return (
    <View style={{ flex:1 }}>
      ...
      <View style={styles.container}>
        ...
        {['rut','nombre','apellido','correo_electronico','telefono','fecha_nacimiento'].map(campo => (
          <TextInput
            key={campo}
            style={styles.input}
